"""reject command — reject a HITL-pending change with a required reason."""
...
def register(app: typer.Typer) -> None:
    ...
    def reject(
        ...
        """Reject a HITL-pending change.

        Moves the item from queue/hitl/pending/ to queue/hitl/rejected/ and
        writes a hitl_action audit record.
        """
        ...
        try:
            queue.reject(full_id, reviewer=reviewer_name, reason=reason)
        except Exception as exc:
            err_console.print(f"[red]Reject failed:[/red] {exc}")
