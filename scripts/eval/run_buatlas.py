#!/usr/bin/env python3
"""Run the BUAtlas eval harness.

Usage:
    .venv/bin/python scripts/eval/run_buatlas.py
    .venv/bin/python scripts/eval/run_buatlas.py --runs 5 --out-dir audit/eval/my-run
...
def main() -> int:
    parser = argparse.ArgumentParser(description="BUAtlas eval harness")
    ...
    expectations = [e for e in EXPECTATIONS if e.agent == "buatlas"]
    print(
        f"\nBUAtlas eval — {len(expectations)} cases, {args.runs} runs each"
        f"\nNote: each case first runs SignalScribe once (setup) — not counted in BUAtlas cost."
        f"\nOutput: {out_dir}\n"
    )
    ...
    md_path, json_path = write_agent_report(results, "buatlas", args.runs, out_dir)
    print(f"\nReport: {md_path}")
